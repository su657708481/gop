...
//				System.out.println("simi_node: "+simi_node.getNodeName());
	...
	public static Node lookupXmlNode(Document doc, String gname) {
		NodeList nodeList = doc.getElementsByTagName("granule");
		if (nodeList != null) {
	...
	public ArrayList<String> lookupShadowsOfClass(String classname) {
		HashSet<String> superClassSet = new HashSet<String>();
		ClassLoader classLoader = ClassLoader.getSystemClassLoader();
		Class curClass = null;
		try{
			curClass = classLoader.loadClass(classname);
		}
		catch(Exception e) {
			e.printStackTrace();
			return null;
		...
		while (curClass != null) {
			superClassSet.add(curClass.getSimpleName());
			curClass = curClass.getSuperclass();

		}
		ArrayList<String> result = new ArrayList<String>();
		lookupShadowsOfClass(doc.getDocumentElement().getFirstChild(), superClassSet, result);
		return result;
	}

	private void lookupShadowsOfClass(Node node, HashSet<String> superClassSet, ArrayList<String> result) {
	 	final String baseAttrName = "shadow_class";
		if (node.getNodeType() == Node.ELEMENT_NODE) {
			Element elem = (Element)node;

			for (int i = 0; ; ++i) {
				String attrName = baseAttrName + i;
				if (!elem.hasAttribute(attrName))
					break;
				String shadow_class = elem.getAttribute(attrName);
				if (superClassSet.contains(shadow_class.split("_")[1]))
					result.add(shadow_class);
			}

			if (elem.getTagName().equals("granule"))
				for (Node child = node.getFirstChild(); child != null; child = child.getNextSibling())
					lookupShadowsOfClass(child, superClassSet, result);
		}
	}
