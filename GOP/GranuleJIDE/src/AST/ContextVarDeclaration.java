  ...
  public Stmt createContextVarBlockStmt(){
        boolean isObject = false;
	     ...
	     if(hasInit()){
	      if(getTypeAccess().type().isString()){
	      ...
	      else if (getTypeAccess().type().isPrimitive()) {
              ...
              isObject = true;
              l.add(new CastExpr(new TypeAccess("java.io.Serializable"), getInit()));
          }
	     } else {
	         if(getTypeAccess().type().isString()){
	    ...
	    // AbstractDot d=new AbstractDot(ta,pn);
	     ...
	     //d.replaceLast(m);
      String methodname=new String(isObject ? "addContextByObject" : "addContext");
