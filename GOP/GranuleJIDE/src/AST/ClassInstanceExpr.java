...
if(isFinal && num == state().boundariesCrossed) type_computed = true;
  ...
  public ASTNode rewriteTo() {
    if (GranuleOptions.enableRewriteNew && isSourceFile()) {
      state().duringMethodTransform++;
      ASTNode result = rewriteNew();
      state().duringMethodTransform--;
      return result;
    }
  ...
  ASTNode rewriteNew() {
    List<Expr> newInstanceArgs = new List<Expr>();
    //newInstanceArgs.add(type().createQualifiedAccess().qualifiesAccess(new ClassAccess()));
    TypeDecl t = null;
    if (isRedirectType() && redirectType != null)
      t = redirectType;
    else
      return super.rewriteTo();

    newInstanceArgs.add(t.createQualifiedAccess().qualifiesAccess(new ClassAccess()));
    List<Expr> argList = getArgList().fullCopy();
    List<Expr> argTypeList = new List<Expr>();
    for (Expr e : argList)
      argTypeList.add(e.type().createQualifiedAccess().qualifiesAccess(new ClassAccess()));
    newInstanceArgs.add(new ArrayCreationExpr(new ArrayTypeAccess(new TypeAccess("Class")),new Opt(new ArrayInit(argTypeList))));
    newInstanceArgs.add(new ArrayCreationExpr(new ArrayTypeAccess(new TypeAccess("Object")),new Opt(new ArrayInit(argList))));
    return new CastExpr(type().createQualifiedAccess(), new TypeAccess("ObjectAgent").qualifiesAccess(new MethodAccess("newInstance", newInstanceArgs)));
  }
}
