   ...
   */  private AbstractDot rewriteRule0() {
{
	//TODO:上下文对象化 by WUWENBIN
				       ...
				       l.add(getSource());
                         ...
                         ddd.replaceLast(mm);
                         l.add(ddd);
                       } else if (/*sourceType().implementedInterfaces().contains("java.io.Serializable")*/true){
				         isObject = true;
				         l.add(new CastExpr(new TypeAccess("java.io.Serializable"), getSource()));
                       } else {
                         System.err.println("Error: Context variable type must be serializable " + typename + "!");
                       }
