...
boolean isRegisterToClassInstance=false;
  ...
  public boolean containsBaseClass(ClassDecl cd){
	    return redirectClassToShadowClass.containsKey(cd);
	}
  /**
   * @ast method 
   * @aspect BaseClassMapping
  ...
  private HashMap<ClassDecl, ShadowClassDecl> redirectClassToShadowClass= new HashMap<ClassDecl, ShadowClassDecl>();

    public ShadowClassDecl getRedirectType(ClassDecl cd){
	   if(redirectClassToShadowClass.containsKey(cd)) {
         useCompositeShadow(cd);
         return redirectClassToShadowClass.get(cd);
       }
       return null;
	}
    ...
    // TODO: 未考虑多个粒的影子类注册到同一个类的情况
    private HashMap<ClassDecl, ShadowClassDecl> directClassMapShadow = new HashMap<ClassDecl, ShadowClassDecl>();

    private ArrayList<ShadowClassDecl> getAllShadowsOfClass(ClassDecl decl) {
      ArrayList<ShadowClassDecl> shadows = new ArrayList<ShadowClassDecl>();
      System.out.println(decl.fullName());

      ClassDecl current = decl;
      while (current != null) {
          if (directClassMapShadow.containsKey(current))
          {
              shadows.add(directClassMapShadow.get(current));
              System.out.println(" " + directClassMapShadow.get(current).name());
          }
  ...
  private HashMap<String, ShadowClassDecl> compositeShadows = new HashMap<String, ShadowClassDecl>();

    class CompositeShadowRecord
    {
      String name;
      String base;
      ShadowClassDecl origin;
    }
  private HashMap<ClassDecl, ArrayList<CompositeShadowRecord>> compositeShadowOfClass = new HashMap<ClassDecl, ArrayList<CompositeShadowRecord>>();
    private ShadowClassDecl buildShadowComposite(ArrayList<ShadowClassDecl> shadows, ClassDecl cd) {
      if (shadows.size() == 1 && shadows.get(0).seedClass() == cd)
        return shadows.get(0);

      String parentId = cd.getID();
      String gName = cd.fullName();

      MessageDigest md = null;
      ShadowClassDecl last = null;
      ArrayList<CompositeShadowRecord> shadowList = new ArrayList<CompositeShadowRecord>();
      compositeShadowOfClass.put(cd, shadowList);
      try {
        md = MessageDigest.getInstance("MD5");
        for (ShadowClassDecl shadow : shadows)
        {
          ShadowClassDecl composite_shadow = shadow.fullCopy();
          gName = gName + ";" + composite_shadow.name();
          String newName = "g" + toHex(md.digest(gName.getBytes())) + "_" + cd.getID();
          composite_shadow.setCompositeName(newName);
          composite_shadow.resetConstructors();
          //composite_shadow.setCompositeBase(cd.constantPoolName());
          composite_shadow.flushCache();
          System.out.println(newName + "       " + shadow.name());

          compositeShadows.put(newName, composite_shadow);
          CompositeShadowRecord record = new CompositeShadowRecord();
          record.name = newName;
          record.origin = shadow;
          record.base = parentId;
          shadowList.add(record);

          parentId = newName;
          last = composite_shadow;
        }
  ...
  private HashSet<ClassDecl> willOrGeneratedCompositeShadows = new HashSet<ClassDecl>();
    private ArrayList<ClassDecl> generateCompositeShadowQueue = new ArrayList<ClassDecl>();

    public void useCompositeShadow(ClassDecl cd)
    {
      if (willOrGeneratedCompositeShadows.contains(cd) || !compositeShadowOfClass.containsKey(cd))
        return;
      willOrGeneratedCompositeShadows.add(cd);
      generateCompositeShadowQueue.add(cd);
    }
    int mytimesx = 0;
    public void generateCompositeShadows()
    {
      System.out.println(mytimesx++);
      for (ClassDecl cd : generateCompositeShadowQueue)
      {
        cd.generateClassfile();
        for (CompositeShadowRecord record : compositeShadowOfClass.get(cd))
        {
          ClassPool cp = new ClassPool(ClassPool.getDefault());
          record.origin.generateClassfile();
          try {
            cp.insertClassPath(record.origin.destinationPath());
            CtClass c = cp.get(record.origin.constantPoolName());
            c.setName(record.name);
            c.setSuperclass(cp.get(record.base));
            c.writeFile(record.origin.destinationPath());
      ...
      generateCompositeShadowQueue.clear();
    }
        ...
        while (cuIter.hasNext()) {
          ...
          if (cu.fromSource()) {
            for (TypeDecl td : cu.getTypeDeclListNoTransform()) {
              if (td instanceof ShadowClassDecl) {
                ShadowClassDecl scd = (ShadowClassDecl) td;
                ClassDecl cd = scd.seedClass();
                directClassMapShadow.put(cd, scd);
              }
            }
          }
        }
        cuIter = compilationUnitIterator();
        while (cuIter.hasNext()) {
          CompilationUnit cu = cuIter.next();
          if (cu.fromSource()) {
            for (TypeDecl td : cu.getTypeDeclListNoTransform()) {
              if (td instanceof ClassDecl) {
                ...
                ArrayList<ShadowClassDecl> shadows = getAllShadowsOfClass(cd);
                if (shadows.size() != 0)
                    redirectClassToShadowClass.put(cd, buildShadowComposite(shadows, cd));
              }
            }
          }
        }
...
//		sortClassMapping();
//		firstVisit=true;
//		GranuleTree tree = CreateGranuleTree();
//
//		cuIter = compilationUnitIterator();
//		while (cuIter.hasNext()) {
//		    CompilationUnit cu = cuIter.next();
...
//                        if(td instanceof ShadowClassDecl){
//                            ShadowClassDecl scd=(ShadowClassDecl)td;
//                            TypeAccess g= (TypeAccess)scd.getGranuleAccess();
//                            tree.find(g.name()).addShadowClass(scd);
//                        }
//                    }
//                }
//            }
//            displayGranuleTree(tree.rootElement, 0);
//	}
//   }
//
//   public void displayGranuleTree(GranuleTreeNode node, int indent)
//   {
//     for (int i = 0; i < indent; ++i)
//       System.out.print(" ");
//      System.out.print(node.granulename + ": ");
//      for (int i = 0; i < node.shadowClassSet.size(); ++i) {
//           if (i != 0) System.out.print(",");
//          System.out.print(node.shadowClassSet.get(i).name());
//      }
//      System.out.println();
//      for (int i = 0; i < node.children.size(); ++i)
//        displayGranuleTree(node.children.get(i), indent + 4);
//   }
  /**
   * @ast method 
...
if(isFinal && num == state().boundariesCrossed) lookupType_String_String_values.put(_parameters, lookupType_String_String_value);
  ...
  private TypeDecl lookupType_compute(String packageName, String typeName) {
    ...
    //TODO: inject here这里是查找类型的地方
    if (compositeShadows.containsKey(fullName))
      return compositeShadows.get(fullName);
    return null;
  }
  /**
   * @apilvl internal
