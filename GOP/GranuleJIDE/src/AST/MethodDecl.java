...
//==============END========================================
    ...
    public MethodDecl primaryMethod(String signature) {
        ...
        if (isFinal && num == state().boundariesCrossed)
            primaryMethod_String_values.put(_parameters, primaryMethod_String_value);
     ...
     * @apilvl internal
     */
    private boolean isBase_compute() {
        return getModifiers().isBase();
    }
   ...
   */
    ...
    private int resultOffset_compute() {
        return type().isVoid() ? 0 : type().variableSize();
    }
   ...
   */
    ...
    public ASTNode rewriteTo() {
        ...
        boolean a1 = isAbstract();
        boolean a2 = isOverridingMethod();
        if (!m_done() && !isAbstract()) {
            state().duringMethodTransform++;
            if (isOverridingMethod())
                rewriteRule0().rewriteRule1();
            else if (hostType().isClassDecl() && isFromSourceFilesTo() && !isStatic())
                rewriteRule1();
            state().duringMethodTransform--;
            return this;
        }
...
//            System.out.println("test2:"+Thread.currentThread().getName());
    ...
    private MethodDecl rewriteRule1() {
        {
            List<Expr> getLatestObjectArgs = new List<Expr>();
            Expr thisObj = new CastExpr(new TypeAccess("Object"), new ThisAccess("this"));
            getLatestObjectArgs.add(thisObj);
            Expr getLatestObject = new TypeAccess("ObjectAgent").qualifiesAccess(new MethodAccess("getLatestObject", getLatestObjectArgs));

            TypeAccess baseType;
            if (hostType().isShadowClassDecl()) {
                baseType = new TypeAccess(((ShadowClassDecl) hostType()).seedClass().hostType().typeName());
            } else {
                baseType = new TypeAccess(hostType().typeName());
            }
            getLatestObject = new CastExpr(baseType, getLatestObject);
            Stmt objVarStmt = new VariableDeclaration(baseType, "latestObj", getLatestObject);

            Expr callLatestMethod = null;
            List<Expr> callLatestArgs = new List<Expr>();
            List params = getParameterList().fullCopy();
            int len = params.getNumChild();
            for (int i = 0; i < len; ++i)
                callLatestArgs.add(new VarAccess(((ParameterDeclaration) params.getChild(i)).getID()));
            callLatestMethod = new VarAccess("latestObj").qualifiesAccess(new MethodAccess(name(), callLatestArgs));
            ...
            Stmt callLatestMethodStmt;
            if (!isVoid())
                callLatestMethodStmt = new ReturnStmt(new CastExpr(new TypeAccess(returnTypeName), callLatestMethod));
            else
                callLatestMethodStmt = new ExprStmt(callLatestMethod);

            List<Stmt> ls = new List<Stmt>();
            ls.add(objVarStmt);
            IfStmt ifNotLatest = new IfStmt(new NEExpr(new CastExpr(new TypeAccess("Object"), new VarAccess("latestObj")), thisObj), callLatestMethodStmt, new Opt(getBlock().fullCopy()));
            ls.add(ifNotLatest);
            setBlock(new Block(ls));
            return this;
        }
