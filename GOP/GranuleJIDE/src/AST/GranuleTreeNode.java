...
public class GranuleTreeNode extends java.lang.Object {
 ...
 ArrayList<ShadowClassDecl> shadowClassSet;
 ...
 GranuleTreeNode(String granulename,String contextname,GranuleDecl granuledecl){  
        ...
        this.shadowClassSet = new ArrayList<ShadowClassDecl>();
     }
...
public void addShadowClass(ShadowClassDecl decl)
{
    shadowClassSet.add(decl);
}

    public GranuleTreeNode find(String name) {
        if (granulename.equals(name))
            return this;
        for (GranuleTreeNode child : children)
        {
            GranuleTreeNode result = child.find(name);
            if (result != null)
                return result;
        }
        return null;
